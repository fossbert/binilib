...
#' \item{RS}{The running sum}
#' \item{signature}{The input signature}
#' \item{Idx}{Indices of gene set genes in the signature}
#' }
#' @export
gsea2ES <- function(signature, gS, weight = 1){
    ...
    es <- rs[maxabs]
    list(ES = es, RS = rs, signature = signature, idx = idx)
}
...
rea <- function(eset, regulon, minsize=1,maxsize=Inf) {
    ...
    message("\nComputing CITRUS associations of ", length(regulon)," gene sets with ", ncol(eset), " samples")
